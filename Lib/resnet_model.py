...
from tensorflow.keras.layers import (
    Input, Activation, Dense, Flatten, Dropout, Conv3D,
    AveragePooling3D, MaxPooling3D, BatchNormalization, Add
)
    ...
    def f(input):
        if is_first_block_of_first_layer:
            ...
            conv1 = _bn_relu_conv3d(filters=filters, kernel_size=(3, 3, 3),
                                    strides=strides, kernel_regularizer=kernel_regularizer)(input)
...
class Resnet3DBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions, reg_factor, drop_rate=0):
        ...
        pool2 = AveragePooling3D(pool_size=(block_shape[DIM1_AXIS],
                                            block_shape[DIM2_AXIS],
                                            block_shape[DIM3_AXIS]),
                                 strides=(1, 1, 1))(block)
        flatten1 = Flatten()(pool2)
        dense = Dense(units=num_outputs, kernel_initializer="he_normal",
                      kernel_regularizer=l2(reg_factor), activation="softmax")(flatten1)
        model = Model(inputs=input, outputs=dense)
        return model

    @staticmethod
    def build_resnet_101(input_shape, num_outputs, reg_factor, drop_rate=0):
